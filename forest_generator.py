...
class ForestCreator(QtWidgets.QDialog):
    ...
    def create_tree_color_controls(self):
        ...
        layout.addWidget(QLabel("Tree Leaves Color:"), 1, 0)
        self.tree_leaves_color_btn = self.create_color_btn("#0E912A", 
                                                           "tree_leaves")
        layout.addWidget(self.tree_leaves_color_btn, 1, 1)
    ...
    def generate_forest(self):
        ...
        try:
            ...
            for ast, asset_type in enumerate(assets_to_scatter):
                ...
                position = self.find_valid_pos(
                    area_width, area_depth, center_x, center_z, 
                    self.min_spacing.value(), self.placement_attempts.value()
                    )
    ...
    def clear_scene(self):
        try:
            self.clear_previous_scatter()
            forest_patterns = ["Forest_*", "tree_*", "rock_*", "mushroom_*", 
                               "base_tree", "full_mushroom"]

            for pattern in forest_patterns:
            ...
            shaders = cmds.ls("*_shader", "*SGH", type="shadingEngine")
            ...
            self.generate_base_assets()
            self.status_label.setText("Scene cleared successfully")
